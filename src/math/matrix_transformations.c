...
void calculate_transforms(t_fdf *fdf)
{
    ...
    t_mat4 cam_mat;
    t_vec4 cam_up = {0, 1, 0, 0};

    camera_projection(&cam_mat, &fdf->cam_pos, &cam_up, &fdf->cam_look);
    t_mat4 temp;
    identity_matrix(&temp);

    // mat_multiply(&temp, &orth_mat, &cam_mat);

    // mat_multiply(&fdf->projection, &vp_mat, &temp);

    mat_multiply(&fdf->projection, &temp, &cam_mat);
    ...
    // mat_multiply(&fdf->transform_mat, &fdf->orientation,
    // &fdf->transform_mat); mat_multiply(&fdf->transform_mat,
    // &fdf->translation, &fdf->transform_mat);
